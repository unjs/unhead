...
- `undefined`: Script tag will be inserted immediately and usable once the network request is complete.
...
for example on the `requestIdleCallback` hook.
- `Function`: Load the script when the function is called.
