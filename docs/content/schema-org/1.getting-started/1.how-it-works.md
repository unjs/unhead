...
In practice this means you can provide the "global" schema (`WebPage`, `WebSite` and an [identity](/schema-org/guides/identity)) within 
...
make use of `defineBreadcrumb` to have all articles generate [Breadcrumbs](/schema-org/schema/breadcrumb). 
...
For example, having a `document.title` can be inferred as the [WebPage](/schema-org/schema/webpage) `title`.
