...
With [`renderToPipeableStream()`](https://react.dev/reference/react-dom/server/renderToPipeableStream), React can send the document shell before a component inside `<Suspense>` calls `useHead()`. The Unhead transform wraps that component's own return as `<><HeadStream />{jsx}</>`, so its later entry still reaches the page. `HeadStream` has to re-render with the component that called `useHead`; placed as a sibling of it, React evaluates it once before the suspension is detected and never again, and the entry is lost.
