...
```ts
import { createStreamableHead, streamAppWithUnhead } from '@unhead/vue/server'
...
for await (const chunk of streamAppWithUnhead(appStream, template, head)) {
  res.write(chunk)
}
res.end()
```
...
Create a component that emits a marker for head updates:
...
Use `useHead()`{lang="ts"} in async components, then include `HeadStream`{lang="vue"} after the content:
...
<body ${ssr.bodyAttrs}>${html}${ssr.bodyTags}</body>
    ...
    res.set('Content-Type', 'text/html')
    for await (const chunk of streamAppWithUnhead(renderToWebStream(app), template, head)) {
      res.write(chunk)
    }
    res.end()
  }
