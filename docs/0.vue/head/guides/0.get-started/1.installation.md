...
Remove the sunset `@vueuse/head` package before installing `@unhead/vue`. Keeping both packages can install Unhead v1 beside the current version while auto-imports still resolve the old composables.
...
The `<Head>` component has its own entry point:
...
If you use `unplugin-auto-import`, replace the old package preset with Unhead's composable list:
