...
A tag takes the third row when it does not need the head to do its job: JSON-LD, which search engines read anywhere in the document; `noscript`, whose only audience is a client that never runs the patch; and anything you gave a `tagPosition`. `bodyOpen` lands at the body close like the rest, because the body-open slot flushed with the shell.
