...
`tagsHiddenFromBots` holds the subset that only works when it is in the served `<head>`:

| Hidden from bots | Safe to patch |
| --- | --- |
| `<title>` and `titleTemplate` | `link[rel=preload]`, `link[rel=stylesheet]`, other resource hints |
| `link[rel=canonical]`, `alternate`, `amphtml`, `prev`, `next` | ordinary `<script>` and `<style>` |
| `meta[name=description]`, `robots`, `googlebot`, `keywords` | `htmlAttrs`, `bodyAttrs`, `templateParams` |
| Open Graph and Twitter card meta | any tag with a `bodyOpen` or `bodyClose` position |
