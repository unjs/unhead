...
::warning
A browser applies streamed patches. Many crawlers and social scrapers do not: they read the HTML the server sent and never run the patch script. Googlebot does render JavaScript, but link unfurlers and smaller crawlers generally do not, and rendering is a second pass rather than a guarantee. Tags that only work from the served `<head>`, such as canonical links, robots and description meta, Open Graph cards, and JSON-LD, are the ones to keep ahead of the shell.
::
