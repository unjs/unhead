...
```ts
import { type PreparedTemplate, wrapStream } from 'unhead/stream/server'
...
Parses a stable HTML template for reuse by `transformHtmlTemplate()`, `transformHtmlTemplateRaw()`, `renderSSRHeadShell()`, `prepareStreamingTemplate()`, and `wrapStream()`:

```ts
import { prepareTemplate } from 'unhead/stream/server'

const template = prepareTemplate(html)
```
