...
| after the shell renders, and it works from the body | Streamed Body Tags, before `</body>` |

`flushChunk` controls late head patch output. It does not route Streamed Body Tags. `renderStreamEnd()` writes them before `</body>`.
...
Streamed Body Tags take the third row when they do not need the head:
...
Builds the closing HTML for a prepared template. It inserts and clears Streamed Body Tags:
...
Renders and clears Streamed Body Tags. Use this when your response has no prepared template:
...
Create manual drivers with `writesBodyTags: true`. Otherwise, Streamed Body Tags stay in the patch as a fallback.
...
| `script` | `type` is `application/ld+json`, at any position, unless the driver writes the body markup |
