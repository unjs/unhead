...
Vue's `renderToWebStream()` runs setup functions while it constructs the stream, but only as far as it can get synchronously. A component's `useHead()` reaches the shell when it runs before that component's own first `await` **and** every ancestor above it is still synchronous at that moment.

That second half is easy to miss. When a component's `setup()` returns a promise, Vue defers rendering its entire subtree, so a child with no `await` of its own still misses the shell:
...
| How you wrap the stream | Entry before the shell | Entry after the shell |
| --- | --- | --- |
| a framework wrapper, such as `createStreamableHead().wrapStream` from `@unhead/vue/stream/server` | served `<head>` | client-side patch |
| `wrapStream(head, stream, template)` with no `flushChunk` | served `<head>` | **dropped, never sent** |

::warning
The bare three-argument `wrapStream()` call has nowhere to write late entries, so it discards them silently. Use a framework wrapper, pass your own `flushChunk` as the fifth argument, or take over the stream with [Manual Stream Control](#manual-stream-control). The `wrapStream` API entry below covers the callback.
::
...
::warning
A browser applies streamed patches. A bot does not: it reads the HTML the server sent and never runs the patch script. Tags that only work from the served `<head>`, such as canonical links, robots and description meta, Open Graph cards, and JSON-LD, are invisible to bots when they arrive as patches. Keep those entries ahead of the shell.
::
...
The bundler plugin does not validate its `streamKey`, and most framework `/bundler` entry points do not expose the option at all. A key that only the plugin sees fails later, when the server calls `createBootstrapScript()` or `renderSSRHeadShell()`.
