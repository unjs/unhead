...
Entries take one of three paths, and which one depends on `flushChunk`:
...
A renderer that defers all its work until its stream is first pulled registers nothing before the shell, so every tag it sets falls into an "after the shell" row.

A tag takes the third row when it does not need the head to work:
...
- Anything you gave a `tagPosition`. A streamed tag cannot reach `bodyOpen`, because those bytes left with the shell, so it renders at the body close.
...
A browser applies streamed patches. A bot does not: it reads the HTML the server sent and never runs the patch script. So a tag registered after the shell is invisible to it.

`ValidatePlugin` reports those during development, under the rule `streamed-tag-hidden-from-bots`. Production never registers the plugin, so nothing is classified and nothing is paid for.

The rule fires on the **server** head, from the chunk renderer. The bundler plugin only auto-injects `ValidatePlugin` into `createHead()` call sites, and a streaming app calls `createStreamableHead()`, so pass it yourself:
...
const { head, wrapStream } = createStreamableHead({
  plugins: import.meta.dev
    ? [ValidatePlugin({
        // this instance only covers what the client one cannot see
        only: ['streamed-tag-hidden-from-bots'],
        // a distinct key, so a ValidatePlugin you register elsewhere is not
...
Each report carries the `tag`. It also carries the `source` location of the `useHead` call, when the plugin can capture it. Turn the rule off or change its severity like any other:
...
`useHeadSafe` strips some tags, such as a `rel="canonical"` link. The rule skips those. Nothing renders them, so a warning about bot visibility would mislead you.

JSON-LD leaves the stream as real markup when the driver writes it, so the rule stays quiet there. `wrapStream` always does. A driver that builds its own closing HTML promises the same with the `writesMarkup` option.

Everything else is left alone, because a browser applying it later is all it needed: resource hints and stylesheets, ordinary scripts and styles, `htmlAttrs`, `bodyAttrs`, `templateParams`, and anything positioned in the body.

JSON-LD is the one exception to the body rule. Search engines read it anywhere in the document. Moving it out of the head does not save it. Every other reported tag only works from the head.

`useSeoMeta` is covered. It pushes one internal entry. That entry becomes real meta tags during normalization. The rule expands it the same way a render does, so each tag is reported on its own.

Matching ignores case. A `link` matches if any `rel` token is in the list, so `rel="alternate stylesheet"` is reported. Tokens split on any whitespace.
