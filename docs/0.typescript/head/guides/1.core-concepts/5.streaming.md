...
`wrapStream()` renders the shell the moment you call it, from whatever entries exist at that point. Entries pushed later cannot reach the `<head>` that is already on the wire, so they ship as inline patches instead.

This makes the call order load-bearing. Build the application stream **first**, then pass it to `wrapStream()`:

```ts
// correct: the renderer has already run its synchronous pass
const appStream = renderToWebStream(app)
return wrapStream(head, appStream, template)
```

Vue's `renderToWebStream()` runs the synchronous part of every `setup()` while it constructs the stream, so a `useHead()` call made before any `await` is already registered by the time `wrapStream()` sees it. That is what puts titles, canonical links, and JSON-LD in the served `<head>`.

Measured against real Vue SSR:

| Where `useHead` runs | Served `<head>` | Client patch |
| --- | --- | --- |
| `setup()`, before any `await` | yes | no |
| `setup()`, after an `await` | no | yes |
| inside a nested `<Suspense>` boundary | no | yes |
| a renderer that defers work to the first `pull()` | no | yes |

The last row is the one to watch. If your renderer does no work until its stream is pulled, nothing is registered when the shell renders and every tag becomes a patch. Either push the entries you care about before calling `wrapStream()`, or take over the stream yourself with [Manual Stream Control](#manual-stream-control) and write the shell once your renderer has finished its first pass.
...
The stream key becomes a `window.<name>` accessor, so it must be a plain JavaScript identifier: letters, digits, `$`, and `_`, not starting with a digit. Anything else throws. `myKey` and `__unhead__` are fine, `my-key` and `1x` are not. The same rule applies to `streamKey` on `createStreamableHead()` and on the bundler plugin.
