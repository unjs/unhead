...
```ts
import { STREAM_MARKER } from 'unhead/server'

const STREAM_MARKER: string // '/*__UNHEAD_SSR__*/'
```

Marker string used by `streamWithHead` to identify where head updates should be injected.
