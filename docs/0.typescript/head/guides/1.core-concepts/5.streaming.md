...
A Google Tag Manager `<iframe>` does not care where it lands. A `noscript` carrying visible content does: stream an "enable JavaScript" banner and no-JS visitors read it at the foot of the page. If a tag has to sit at the top of the body, register it before the shell renders.
::
