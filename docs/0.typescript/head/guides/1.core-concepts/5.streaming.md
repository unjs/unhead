...
Each report carries the offending `tag` and, where the plugin could capture it, the `source` location of the `useHead` call that registered it. Turn it off or change its severity like any other rule:
...
Tags that `useHeadSafe` strips, such as a `rel="canonical"` link, are not reported: nothing renders them, for bots or browsers, so a warning about bot visibility would be wrong twice over.
...
JSON-LD is the one exception to the body rule. Search engines read it anywhere in the document, so moving it out of the head does not save it. Every other reported tag only carries meaning from the head.

`useSeoMeta` is covered. It pushes a single internal entry that only becomes real meta tags during normalization, and the rule expands it the same way a render does, so the Open Graph, Twitter, and description tags it sets are reported individually.

Matching is case-insensitive, and a `link` matches if any of its `rel` tokens is in the list, so `rel="alternate stylesheet"` is reported. Tokens are split on any whitespace.
