...
`ValidatePlugin` reports these tags during development under `streamed-tag-hidden-from-bots`. The bundler plugin adds the validator in development by default.

Production does not register the plugin, so classification does not run.

The rule runs on the server head when chunks render. The bundler plugin only adds `ValidatePlugin` to `createHead()` calls.

Streaming apps call `createStreamableHead()`. Add a server instance yourself:
...
const { head } = createStreamableHead({
  plugins: import.meta.dev
    ? [ValidatePlugin({
        // Cover the server-only rule.
        only: ['streamed-tag-hidden-from-bots'],
        // Keep this instance beside a full validator.
...
| `script` | `type` is `application/ld+json`, at any position, unless the driver writes body tags |
...
JSON-LD can leave the stream as body tags. `wrapStream()` always writes them. Manual drivers opt in with `writesBodyTags: true`.

The rule stays quiet when the driver writes body tags. Otherwise, patch-only JSON-LD stays hidden from bots and the rule reports it.
...
Matching ignores case. A `link` matches if any `rel` token is listed. Tokens split on any whitespace.
