...
`ValidatePlugin` reports these tags during development as `streamed-tag-hidden-from-bots`. Vite adds a focused instance to development server heads made by `createHead()`.
