...
- anything with `tagPosition: 'bodyOpen'` or `'bodyClose'`, wherever it came from

Matching is case-insensitive, and a `link` matches if *any* of its space-separated `rel` tokens is in the list, so `rel="alternate stylesheet"` is flagged.
