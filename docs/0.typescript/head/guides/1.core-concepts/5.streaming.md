...
Normalizes the entries that have not been flushed yet, without rendering or clearing them.

Once the shell `<head>` is on the wire, pending entries can only be delivered as client-side patches. Clients that do not run JavaScript never see them. Use this to check what the next `renderSSRHeadSuspenseChunk()` call will defer, and warn during development when an SEO-critical tag will not reach the served HTML:

```ts
import { getPendingTags, renderSSRHeadSuspenseChunk } from 'unhead/stream/server'

const deferred = getPendingTags(head).filter(tag => !tag.tagPosition?.startsWith('body'))
if (import.meta.dev && deferred.length) {
  console.warn(`Head tags delivered as client patches: ${deferred.map(t => t.tag).join(', ')}`)
}
const chunk = renderSSRHeadSuspenseChunk(head)
```

Entry options are applied to each tag, so `tagPosition` tells you whether a tag was bound for the head or the body. Tags are freshly normalized on every call, so mutating them cannot affect a later render.
