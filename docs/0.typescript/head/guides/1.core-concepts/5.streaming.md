...
Inspects the entries that have not been flushed yet, without rendering or clearing them.

Once the shell `<head>` is on the wire, pending entries can only be delivered as client-side patches. A browser applies them. A bot does not: it reads the HTML the server sent, and never runs the patch script.

```ts
import { inspectStreamedTags, renderSSRHeadSuspenseChunk } from 'unhead/stream/server'

const { pendingTags, tagsHiddenFromBots } = inspectStreamedTags(head)

if (import.meta.dev && tagsHiddenFromBots.length) {
  const names = tagsHiddenFromBots.map(tag => tag.tag).join(', ')
  console.warn(`[unhead] Bots will not see these streamed tags: ${names}`)
}

const chunk = renderSSRHeadSuspenseChunk(head)
```

`pendingTags` holds every unflushed entry, normalized. Entry options are applied to each tag, so `tagPosition` tells you whether a tag was bound for the head or the body.

`tagsHiddenFromBots` holds the subset that only works when it is in the served `<head>`. A tag is flagged when it is not body-positioned and matches one of these:
...
| `meta` | `property` (falling back to `name`) starts with `og:`, `twitter:`, `article:`, `book:`, `profile:`, `fb:`, or `al:` |
| `link` | any `rel` token is `canonical`, `alternate`, `amphtml`, `prev`, `next`, `author`, or `license` |
| `script` | `type` is `application/ld+json`, at any position |

Everything else is treated as safe to deliver as a patch, because a browser applies it and a bot has no use for it:

- resource hints and stylesheets: `link[rel=preload]`, `prefetch`, `preconnect`, `dns-prefetch`, `modulepreload`, `stylesheet`, `icon`, `manifest`
- ordinary `<script>` and `<style>`, and `<noscript>`
- `htmlAttrs`, `bodyAttrs`, `templateParams`
- anything with `tagPosition: 'bodyOpen'` or `'bodyClose'`, *except* JSON-LD

JSON-LD is the exception because search engines read it anywhere in the document, so moving it to the body does not save it. Every other flagged tag only carries meaning from the head, so one placed in the body was never going to be read.

Matching is case-insensitive, and a `link` matches if *any* of its `rel` tokens is in the list, so `rel="alternate stylesheet"` is flagged. Tokens are split on any whitespace.

`useSeoMeta` is covered. It pushes one internal entry that only becomes real meta tags during normalization, and `inspectStreamedTags` runs that step, so the Open Graph, Twitter, and description tags it sets are classified like any others.

Tags are normalized fresh on every call and are never cached, so mutating them cannot affect a later render.
