...
Vue's `renderToWebStream()` runs the synchronous part of every `setup()` while it constructs the stream, so a `useHead()` call made before that component's first `await` is already registered when the shell renders. Measured against real Vue SSR, that holds even for a component nested inside a `<Suspense>` boundary: what matters is whether the call runs before the component's own first `await`, not where the component sits in the tree.
