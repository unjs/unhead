...
- Anything you gave a `tagPosition`. You already asked for the body.

::warning
A streamed tag cannot reach `bodyOpen`. Those bytes left with the shell, so it renders at the body close.

Most `noscript` input is a Google Tag Manager `<iframe>` or a tracking pixel, which does not care where it lands. A `noscript` holding visible content does care. Write that one in your component template instead. It streams in place, and no-JS visitors read it where you put it.
::
