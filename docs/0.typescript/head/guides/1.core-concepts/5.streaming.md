...
`ValidatePlugin` reports those during development, under the rule `streamed-tag-hidden-from-bots`. The bundler plugin already injects it in dev, so there is usually nothing to set up. Production never registers the plugin, so nothing is classified and nothing is paid for.

To wire it up yourself, or to route the reports somewhere other than the console:
