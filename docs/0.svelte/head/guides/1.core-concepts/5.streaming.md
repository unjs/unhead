...
export default {
  plugins: [
    // IMPORTANT: unheadSveltePlugin must come BEFORE sveltekit()
    unheadSveltePlugin(),
    sveltekit(),
  ],
}
...
::warning
Plugin order matters! `unheadSveltePlugin`{lang="ts"} must be placed **before** `sveltekit()`{lang="ts"} or `svelte()`{lang="ts"} in the plugins array.
::
...
::note
**Svelte 5 Note:** Svelte 5's `render()`{lang="ts"} function is synchronous and returns HTML immediately. To use streaming infrastructure, wrap the rendered HTML in a ReadableStream. This enables client hydration via the streaming queue even though server rendering is sync.
::
...
export function render(url: string, template: string) {
  ...
  // Set up context for Svelte components
  const context = new Map()
  context.set(UnheadContextKey, head)

  // Svelte 5's render is synchronous
  const rendered = _render(App, {
    props: { url },
    context,
  })

  // Convert sync render to ReadableStream for streaming infrastructure
...
// 2. Write Svelte content
res.write(rendered.html)
...
The Vite plugin injects `HeadStream`{lang="ts"} into your templates, which emit head updates as await blocks resolve.
...
// Server returns SvelteStreamableHeadContext
...
| Return | Description |
|--------|-------------|
| `head` | The Unhead instance to set in context via `context.set(UnheadContextKey, head)` |
| `wrapStream` | Wraps a ReadableStream to handle shell + template injection |

### UnheadContextKey

```ts
import { UnheadContextKey } from '@unhead/svelte/stream/server'
...
Symbol used to provide the head instance to Svelte components via context.
