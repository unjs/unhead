...
If you prefer not to use the Vite plugin, use the `HeadStreamScript`{lang="ts"} function:
...
function HeadStreamScript(): string
```

Returns a script tag string for head updates during streaming. Use with `{@html HeadStreamScript()}`{lang="svelte"}.
