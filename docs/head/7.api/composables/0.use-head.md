...
2. When the document head is being rendered (client-side or during SSR), all queued entries are:
   - Resolved (including any functions or reactive values)
...
This queue-based approach supports deduplication and priority-based rendering while keeping tag resolution synchronous.
