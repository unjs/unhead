...
Reusable browser triggers are available from a separate, tree-shakable entry:
...
The entry also exports `createScriptTriggerTimeout()` and
`createScriptTriggerServiceWorker()`. They install browser listeners only when
Unhead invokes the trigger on the client, and return cleanup to the owning
script or consumer scope.
