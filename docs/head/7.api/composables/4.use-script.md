...
Put `loader` on a keyed resource when an SDK comes from `import()` or another
client-only transport. Unhead will not render a script or preload tag. Triggers,
per-head dedupe, proxy queues, and cancellation work like URL scripts.
