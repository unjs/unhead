...
Some SDKs fire a separate callback after the script element loads. Return a
promise from `use()` to keep `load()` and `onLoaded()` pending until that API is
ready. The supplied signal aborts when the script fails or is removed, so the
adapter can release listeners and global callbacks.

```ts
const script = useScript('https://example.com/sdk.js', {
  use: ({ signal }) => new Promise((resolve, reject) => {
    window.addEventListener(
      'external-sdk:ready',
      () => resolve(window.ExternalSDK),
      { once: true, signal }
    )
