...
### Asynchronous API readiness

Some SDKs fire a separate callback after the script element loads. Use
`waitFor()` to keep `load()` and `onLoaded()` pending until that API is ready.
It rejects on script failure or removal and runs the cleanup returned by its
setup function when readiness settles or the lifecycle is aborted.

```ts
const script = useScript('https://example.com/sdk.js', {
  use: ({ waitFor }) => waitFor((resolve) => {
...
Use a keyed resource with `loader` for an SDK loaded through `import()` or
another client-only transport. It follows the same triggers, shared per-head
identity, proxy queue, readiness, and cancellation lifecycle without rendering
a script or preload tag.

```ts
let sdk: typeof import('example-sdk') | undefined

const script = useScript({ key: 'example-sdk' }, {
  trigger: 'manual',
  loader: async ({ signal }) => {
...
Dynamic imports cannot be cancelled. Check the supplied signal before SDK
initialization when removal must prevent late side effects.
...
Cleanup returned by a function trigger runs when loading starts, its returned
disposer is called, or the script is removed. A consumer scope also calls that
disposer when the scope is released.
...
the registrations and exposes an abort signal without removing the shared
script when it is disposed.

For compatibility, `useScript()` returns the cached shared instance by default.
Opt into a consumer-owned handle with `scope: true` and tie resource cleanup to
its signal:

```ts
import { useScript } from 'unhead/scripts'

const script = useScript(head, 'https://example.com/widget.js', {
  scope: true,
  use: () => window.Widget
})
...
script.onLoaded((api) => {
  ...
  script.signal.addEventListener('abort', () => widget.destroy(), { once: true })
})
```

The Vue integration disposes an opted-in scope with its component lifecycle.
Without a framework lifecycle, dispose it explicitly:

```ts
const script = useScript(head, 'https://example.com/widget.js', { scope: true })
...
Scoped return values also provide the shared `script`, an abort `signal`, and
`dispose()` for consumer-owned registrations and resources.
