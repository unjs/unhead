...
Cleanup returned by a function trigger runs when loading starts, the owning
framework scope is disposed, or the script is removed.
...
scope.onLoadedEffect(async (api, { signal }) => {
