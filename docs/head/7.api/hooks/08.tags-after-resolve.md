  ...
  hooks: {
    'tags:afterResolve': ({ tags }) => {
      for (const tag of tags) {
        if (tag.tag === 'script' && integrityManagedScripts.has(tag.attrs.src) && !tag.attrs.integrity)
          console.warn(`Integrity-managed script has no integrity attribute: ${tag.attrs.src}`)
      }
    }
  }
})
