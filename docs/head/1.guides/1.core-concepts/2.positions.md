...
::warning
Streaming SSR cannot honour `bodyOpen` for a tag registered after the shell flushes. Those bytes are already sent, so the tag renders at the body close. See [Streaming](/docs/typescript/head/guides/core-concepts/streaming).
::
