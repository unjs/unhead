...
::warning
Streaming SSR cannot honour `bodyOpen` for a tag registered after the shell flushes. Those bytes are already sent, so the tag renders at the body close. For markup that must sit at an exact spot in the body, write it in your component template rather than `useHead`. See [Streaming](/docs/typescript/head/guides/core-concepts/streaming).
::
