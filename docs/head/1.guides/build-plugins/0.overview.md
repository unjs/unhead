...
- **Static precompilation (experimental)**: Normalize fully static `useHead()` and `useSeoMeta()` calls during the build
...
Unhead({
  ...
  // Normalize static head entries at build time
...
| `experimental.precompile` | `boolean` | disabled | Precompile fully static `useHead()` and `useSeoMeta()` inputs |
...
Static precompilation moves head-input normalization out of server runtime bundles. Client-targeted builds skip the phase so they do not ship the carrier. It is opt-in while the serialized format is experimental:
...
The transform only handles object and array literals made from JSON-compatible values. If a call contains a variable, function, spread, computed key, or other dynamic expression, the whole call stays on the normal runtime path. `useHeadSafe()`, `useServerHead()`, and their SEO variants are not precompiled. Direct static patches on a `const` binding returned by `useHead()` are also precompiled.

Entry options and `entries:normalize` hooks still run at runtime. The compact entry is finalized through the same weighting, identity, caching, and invalidation path as other normalized entries, including the pre-normalized default init entry.

Custom `propResolvers` must declare `_static: true` when they leave plain static values unchanged. Resolving a precompiled entry throws a named error in every environment when an incompatible resolver is present, preventing silent differences from the normal runtime path.

All-static core SSR applications can select the strict runtime entry to let the bundler remove the dynamic input normalizer:

```ts
import { createHead, renderSSRHead } from 'unhead/precompiled/server'
```

The strict entry throws if any dynamic or untransformed application entry reaches resolution. This includes raw `createHead({ init })` entries, plugins or hooks that push raw inputs, observed `useSeoMeta()` results, and dynamic patches. The strict entry is not yet a replacement for framework server entries; keep using `unhead/server` (or the framework server entry) whenever static and dynamic inputs are mixed.
