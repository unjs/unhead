...
Static precompilation moves head-input normalization out of the runtime bundle. It is opt-in while the serialized format is experimental:
...
The transform only handles object and array literals made from JSON-compatible values. If a call contains a variable, function, spread, computed key, or other dynamic expression, the whole call stays on the normal runtime path. `useHeadSafe()`, `useServerHead()`, and their SEO variants are not precompiled.
...
Custom `propResolvers` must declare `_static: true` when they leave plain static values unchanged. Development builds throw a named error for an incompatible resolver; production builds treat the compiled tags as final.
