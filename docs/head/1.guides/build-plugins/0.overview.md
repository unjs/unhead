...
Install your framework's Unhead package and the optional build tooling before importing a `/vite` or `/bundler` entry.

For Vue:

```bash
pnpm add -D @unhead/bundler unplugin
```

For React, Solid.js, or Svelte:

```bash
pnpm add -D @unhead/bundler magic-string oxc-walker unplugin
```

If you use a `/vite` or `/bundler` entry, add the plugin to your Vite config:

Runtime-only projects do not need these packages or the plugin configuration below.
