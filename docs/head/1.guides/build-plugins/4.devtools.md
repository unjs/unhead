...
**Quick Answer:** The Unhead devtools integration adds a panel to [Vite DevTools](https://github.com/vitejs/devtools) showing all active head tags, entries, SEO overview, plugins, and source file locations. Enabled by default in dev mode; install the `@vitejs/devtools-kit` peer to activate the UI.
...
The devtools panel requires the `@vitejs/devtools-kit` peer dependency. The UI itself ships pre-built inside `@unhead/bundler` and is served from there at dev time, so there's nothing else to install. The peer is optional; the plugin degrades gracefully if it's missing.
...
3. **DevTools panel** - a static app pre-built into `@unhead/bundler/dist`, served at `/__unhead/` and registered as a Vite DevTools dock panel
