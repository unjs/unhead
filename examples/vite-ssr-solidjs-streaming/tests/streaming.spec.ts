...
test.describe('SolidJS Streaming SSR with Unhead', () => {
  // ============================================
  // SHELL & INITIAL STATE
  // ============================================
  ...
  test.describe('Streaming behavior', () => {
    ...
    test('all components render', async ({ page }) => {
      ...
      await expect(page.locator('.hero-banner')).toBeVisible({ timeout: 3000 })
      await expect(page.locator('.product-card').first()).toBeVisible({ timeout: 3000 })
      await expect(page.locator('.reviews-section')).toBeVisible({ timeout: 5000 })
      await expect(page.locator('.newsletter')).toBeVisible({ timeout: 5000 })
    })
  })
  ...
  test.describe('Hydration', () => {
    ...
    test('no unexpected console errors during streaming and hydration', async ({ page }) => {
      ...
      // Filter out favicon 404 errors, hydration mismatches (expected during streaming), and WebSocket errors
