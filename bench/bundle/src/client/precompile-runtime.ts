import { createHead, useHead, useSeoMeta } from 'unhead/precompiled/client'

// Bundled twice from this exact source. The disabled build uses the normal
// client runtime; the enabled build lowers every literal to a DOM-ready plan.
export function mountStaticHead() {
...
export function unmountStaticHead() {
  const mounted = mountStaticHead()
