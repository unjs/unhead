...
const clientColdCpu = comparison('precompile-client-cold-adoption-cpu')
const clientColdHeap = comparison('precompile-client-cold-adoption-heap')
const serverUniqueCpu = comparison('precompile-server-unique-e2e-cpu')
const serverSnapshotCpu = comparison('precompile-server-snapshot-e2e-cpu')
const clientCsrCpu = comparison('precompile-client-csr-e2e-cpu')
const clientOffHeap = bench('off', 'precompile-client-e2e-heap')
const clientOnHeap = bench('on', 'precompile-client-e2e-heap')
if (!createCpu || !resolveCpu || !cpu || !heap || !offHeap || !onHeap || !clientCpu || !clientHeap || !clientColdCpu || !clientColdHeap || !serverUniqueCpu || !serverSnapshotCpu || !clientCsrCpu || !clientOffHeap || !clientOnHeap)
...
const clientNonePath = path.join(dist, 'precompile-client-runtime-none/client/precompile-none.mjs')
const clientNoneControlPath = path.join(dist, 'precompile-client-runtime-none/client/precompile-none-control.mjs')
const clientNone = fs.readFileSync(clientNonePath)
const clientNoneControl = fs.readFileSync(clientNoneControlPath)
const failures = []
...
if (!clientNone.equals(clientNoneControl))
  failures.push(`client none profile retained head-attributable bytes (${clientNone.length} B vs ${clientNoneControl.length} B control)`)
if (/unhead|querySelectorAll|createElement|import\(/.test(clientNone.toString()))
  failures.push('client none profile retained a head runtime or async import')
