 ...
 */
export function renderDOMHead<T extends Unhead<any>>(head: T, options: RenderDomHeadOptions = {}) {
  const dom: Document | undefined = options.document || head.resolvedOptions.document
  if (!dom || !head.dirty)
    return

  const beforeRenderCtx: DomBeforeRenderCtx = { shouldRender: true }
  ...
  if (!state) {
    ...
    for (const key of ['body', 'head']) {
      ...
      for (const c of children) {
        ...
        // only read innerHTML for tags that use it to avoid unnecessary DOM access
        if (tag === 'script' || tag === 'style' || tag === 'noscript') {
          props.innerHTML = c.innerHTML
        }
  ...
  function trackCtx({ id, $el, tag }: RenderTagContext & { $el: Element }) {
  ...
  const pending: RenderTagContext[] = []
