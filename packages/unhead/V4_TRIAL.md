...
A 2026-08-05 audit found the compression proxy joins mapped segments with a newline, adding one synthetic byte per segment (3,211 segments v4, 4,831 v3), so both absolute numbers are inflated. Byte-exact span concatenation measures 6,081 B gzip for v4 vs 8,039 for v3; in-context excision (gzip of the chunk minus gzip of the chunk with Unhead spans removed) measures 6,033 vs 8,076. The v4 saving of roughly 2.0 kB gzip is robust across all three methods. All Unhead client code lives in one shared chunk per build (104 chunks total each).
